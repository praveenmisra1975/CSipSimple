...
public class OutgoingCallListFragment extends CSSListFragment {
    ...
    private boolean placeCall(Cursor c) {
        ...
        long accountId = c.getLong(c.getColumnIndexOrThrow(SipProfile.FIELD_ID));
        if(accountId > SipProfile.INVALID_ID) {
            // Extra check for the account id.
            if(service == null) {
                return false;
            }
            boolean canCall = c.getInt(c.getColumnIndexOrThrow(AccountsLoader.FIELD_STATUS_OUTGOING)) == 1;
            if(!canCall) {
                return false;
            }
            try {
                String toCall = c.getString(c.getColumnIndexOrThrow(AccountsLoader.FIELD_NBR_TO_CALL));
            ...
            if(c.getCount() == 1 && canCallOtherAccounts) {
                ...
                do {
                    if(accountToCall != SipProfile.INVALID_ID) {
                        ...
                        if(accountToCall == c.getLong(c.getColumnIndexOrThrow(SipProfile.FIELD_ID))) {
                            if(placeCall(c)) {
                                c.close();
                                callMade = true;
                                return;
                    ...
                    } else if((c.getInt(c.getColumnIndexOrThrow(AccountsLoader.FIELD_FORCE_CALL)) == 1) && canCallOtherAccounts) {
