...
public class OutgoingCall extends BroadcastReceiver {
	...
	public void onReceive(Context aContext, Intent intent) {
	    context = aContext;
		Logger.debug(THIS_FILE,"on receive of outgoing call Call ");
		...
		if(!Intent.ACTION_NEW_OUTGOING_CALL.equals(action)) {
		    Logger.debug(THIS_FILE, "Not launching with correct action ! Do not process");
		    setResultData(number);
            return;
		}
		...
		if ( ignoreNext.equalsIgnoreCase(number) || 
			...
			Logger.debug(THIS_FILE, "Our selector disabled, or Mobile chosen in our selector, send to tel");
			ignoreNext = "";
			setResultData(number);
			return;
		}
		...
		if (action.equals(Intent.ACTION_NEW_OUTGOING_CALL) ) {
	        ...
	        Logger.debug(THIS_FILE, "We have " + potentialHandlers.size() + " potential handlers");
			...
			if(prefsWrapper.isValidConnectionForOutgoing() || potentialHandlers.size() > 1) {
	            ...
	            if(gsmCallHandlerId != SipProfile.INVALID_ID) {
	                if(Filter.isMustCallNumber(aContext, gsmCallHandlerId, number)) {
	                    Logger.debug(THIS_FILE, "Filtering to force pass number along");
				...
				Logger.debug(THIS_FILE, "Start outgoing call chooser for CSipSimple");
		...
		Logger.debug(THIS_FILE, "Can't use SIP, pass number along");
		// Pass the call to pstn handle
		setResultData(number);
		return;
	}
